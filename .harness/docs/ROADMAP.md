...
| 测试 | `backend/tests/` **729 pytest** (R6-A Phase 5 收尾时活跃基线,2026-06-29 `D:\python3.11\python.exe -m pytest tests/ --collect-only` 实测;R5-E 收尾时为 683,R6-A Phase 2 收尾时为 723,R6-A Phase 5 收尾时为 729;计算口径沿用 R5-E 683 baseline + **33 R6-A Phase 1** (NEW FILE `test_interview_agent.py` 23 case: TestSessionLifecycle 3 + TestGapSelection 6 + TestSlotExtractionRules 5 + TestDraftCard 4 + TestStateMachine 3 + TestTracePrivacy 2;NEW FILE `test_interview_api.py` 10 case: TestStartEndpoint 4 + TestReplyEndpoint 4 + TestDraftEndpoint 2) + **7 R6-A Phase 2** (TestSaveCard 3: writes-to-temp / unique-id / round-trip-preview; TestSaveCardEndpoint 4: happy / invalid-mode-400 / missing-field-422 / empty-bullets-422) + **6 R6-A Phase 5** NEW FILE `test_interview_eval.py`: TestEvalHelpers 4 (extract_slots_iteratively / schema_pass_rate / completeness / fabrication_guard) + TestEvalReport 2 (report_contains_no_user_message_text / offline 模式不调 urlopen)) |
