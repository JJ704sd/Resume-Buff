...
- `3b8b9d1` docs(round6-c): summarize live eval result and next steps (R6-C 起点)
...
| 测试 | `backend/tests/` **930 pytest** (R6-C.3 + R6-D 收尾时活跃基线,2026-07-02 `D:\python3.11\python.exe -m pytest tests/ -q` 实测 930 passed in 122.31s;历史快照:R5-E 683 / R6-A Phase 2 723 / R6-A Phase 5 729 / R6-A Phase 4 739 / R6-B Phase 2 768 / R6-B Phase 3 809 / R6-B Phase 4 840 / R6-B Phase 5 863 / R6-C.1 877 / R6-C.2A 889 / R6-C.2B 909 / R6-C.3 930;新增覆盖含 `test_interview_agent.py` slot_meta + policy integration + LLM observability + critical slot integration,`test_interview_api.py` mode switch + draft verification + LLM monkeypatch 路径改 `core.interview_llm` 命名空间,`test_interview_policy.py` confidence-aware policy + critical slot step 4.5,`test_interview_verifier.py`,`test_interview_eval.py` extractor compare + eval contract warnings + product_goal + LLM observability §4.7,**新建 `test_interview_llm.py`** (1090 行, R6-D 平移 6 个 LLM class 共 35 case)) |
...
_最后更新:2026-07-02 R6-C.1+ C.2A+ C.2B+ C.3+ R6-D 全部完成(`91ec8f3 feat(round6-d): 机械拆分 LLM slot 抽取模块 (行为不变)`);R6-C 在 R6-B 已落地可信增强层基础上,补齐 4 块短板:**eval contract 可信度**(`_validate_eval_contract` + §4.5 warnings 6 条 + `product_goal` 标注 10 条 + §4.6 章节 + "schema_pass_rate = 评测合同变化" 口径声明) / **policy gap-specific critical slot**(`INTERVIEW_POLICY_GAP_CRITICAL_SLOTS` + step 4.5, 让 R6-C.1 contract warning 6 条中 5 条三轮内可达) / **LLM 抽取可观测性**(`response_format=json_object` + 2 few-shot + `slot_source_breakdown` + `llm_parse_retry_count` + `llm_to_rules_slot_fallback_count` 3 字段 + §4.7 章节) / **模块边界清晰化**(`core/interview_llm.py` 537 行独立承接 LLM 抽取主链, `interview_agent.py` 2074 → 1499 行, 行为不变重构);后端全量 **930 passed + 0 skipped**,前端 `vue-tsc --noEmit` 0 error + `npm run build` 成功,offline compare 报告(R6-C.3 复跑)隐私自检通过 + §4.7 渲染 `slot_source_breakdown.rules=60 llm=0 mixed=0 llm_rate=0.0 retries=0 fb_to_rules=0`;R6-D 反向依赖禁止(`interview_llm.py` 不得 import `core.interview_agent` / `core.llm_rewriter`)+ 老路径字节级一致(R6-C.3 baseline 零回退);默认路径仍关闭智能抽取(`enable_interview_llm=False`)并保持 rules 行为兼容;真实 LLM eval 仍是手动流程,不进入 pre-push / 默认启动。下一轮 P1 候选:用户在 chat panel 跑 10+ 轮真实对话 + 配置 LLM key 后跑 `scripts/evaluate_interview_agent.py --mode live --extractor compare` 生成 v2 live 报告,作 Phase 4 启动 ground truth(spec §8 product gate);入库前必人工隐私检查(公开脱敏 materials.json / 公开 JD 库 / eval set 内置 / `LLM_BASE_URL` 不含内部路径);若 v2 显示 LLM 质量 delta 明确提升再讨论 R6-D+ LLM prompt 优化 / retry 策略升级 / token 控制 / `PROMPT_VERSIONS` 扩(决策点 D5 突破);否则优先 R5-F prompt rollout(基于 R5-E live A/B 报告决策)。_
