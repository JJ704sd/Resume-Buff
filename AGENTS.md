...
- 后端 `pytest`：Round 4 收尾后有 **283 个用例**（181 R3-G baseline + 1 `test_special_chars_emoji_fullwidth_normalized` + 1 `test_exe_format_raises_unsupported_with_supported_types_in_message` + 7 R3-M.1 MVP：3 个 `test_layout_generates_valid_docx` 参数化 + 4 个 visuals `test_academic_larger_font_size` / `test_internet_smaller_font_size` / `test_internet_has_skill_marker` / `test_bilingual_default_margins` + **23 R3-M.2: 5 个 `TestLayoutConfigSchema` 锁 LAYOUT_CONFIG schema(h1_size_ratio / h2_size_ratio / section_spacing_pt / meta_spacing_pt / item_spacing_pt 必含 + 比例不变量) + 9 个 helper 集成(4 `TestHeadingHierarchy` + 2 `TestSectionSpacing` + 2 `TestBulletSpacing` + 1 `TestMetaSpacing`,验证参数真的写到 docx 上) + 5 个 `TestAcademicRenderer`(dispatch / 无项目名 H2 / 无独立 period meta / 无 summary / education 在 projects 前) + 4 个 `TestReadabilityAcrossLayouts`(body>=9pt / meta>=8pt / 4 间距>=0 / line_spacing [1.15, 1.5])** + **20 R3-M.3: 6 个 `TestAcademicLayout` 锁 academic_layout compact/detailed 分支(default 走 compact / compact 不渲染 H2+meta+summary / detailed 恢复 H2+meta+summary) + 4 个 `TestBilingualHeader`(中文姓名 / 英文姓名 when present / graceful no en when absent / 居中) + 3 个 `TestBilingualEducation`(line / school_en when present / graceful) + 3 个 `TestBilingualProject`(title / title_en when present / graceful) + 2 个 `TestBilingualDispatch`(bilingual 走 _render_bilingual / classic 仍走 _render_classic) + 2 个 `TestMaterialsBilingualSchema`(真实 materials 无 _en 字段 bilingual 不抛异常 + build_sections 透传 _en 字段 schema 锁)** + **19 R3-P: 3 个 `TestSystemPromptV2` 锁 SYSTEM_PROMPT 含 few-shot 示例(基础 + jd_focus)+ 7 条硬性约束 + 5 个 `TestNewSchemaExtraction` 锁新 schema 验证(接受 / 拒绝重复 index / 越界 / 空 text / count 不匹配)+ 3 个 `TestRetryOnInvalid` 锁 retry 成功 / retry 仍失败降级 / 网络错误不 retry + 8 个 `TestSchemaValidationUnit` 锁 _validate_new_schema / _validate_legacy_schema 单元函数** + **8 R4-F: 5 个 `TestFunctionCalling` 锁 tools schema 结构 / 默认不挂 / jd_focus=None 不挂 / 启用挂载 / tool_calls 解析失败降级原文 + 3 个 `TestEvaluateBulletJdMatch` 锁 evaluate_bullet_jd_match 工具函数 surface 命中 + missing 列表** + **12 R4-A: 9 个 `TestAgentLoop` 锁 max_step 上限 / 工具成功路径 / 工具失败回退 / 连续 3 步仍无 output 降级 / trace 格式 / 网络错误不入 loop / max_step=0 走老路径 / 单步单工具约束 / execute_tool_call known+unknown 工具 + 3 个 `TestLogAgentTrace` 锁 trace 写入格式 + 文件创建 + 字段完整性** + **11 R4-M: 8 个 `TestSessionAPI` 锁 create/append/get/clear/上限 10 FIFO/不存在 session_id 静默忽略/空 session_id 静默忽略/唯一性 + 3 个 `TestSessionIntegration` 锁 session_id 拼接到 LLM messages / session=None 不拼接 / 多轮同 session 累积**），**283 passed + 0 skipped**（R3-G 移植自 worktree `eb7e841` + 当前 main R3.5+ borrowed pool / KEYWORD_GROUPS 适配;baiyun_product 第三次复核改 '别投' label 跟 match_score score=33 '低' 一致, 8/8 = 100% 准确率 + R3-M.2 academic 专属 renderer 简化项目 highlights + R3-M.3 激活 bilingual_mode + academic_layout compact/detailed + 6 role _BASELINE_HASHES 因 _en 字段 schema 扩展重算固化 + **R3-P Prompt 工程升级:SYSTEM_PROMPT v2 加 2 个 few-shot 示例(基础改写 + jd_focus)+ 显式 JSON schema `{rewritten: [{index, text}]}` 唯一规范 + 失败 retry 1 次(更严格指令)+ 旧 schema 保留兼容**）
