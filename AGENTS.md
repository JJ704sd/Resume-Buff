...
## Testing instructions
  ...
  - 后端 `pytest`：Round 5-B Phase 2A 收尾后有 **487 个用例**（441 R5-A closeout baseline + 19 R5-B Phase 2A `test_tool_schema.py` + 18 R5-B Phase 2A `test_agent_tools.py` 增量 + 9 R5-B Phase 2A `test_agent_workflow.py` 增量），**487 passed + 0 skipped**（R5-B Phase 2A 在 R5-A closeout 之上加：完整轻量 JSON schema validator 子集 type / required / properties / items / minimum / maximum 零依赖纯 stdlib + context 权限边界 `allow_jd_text` / `allow_materials` / `allow_external_resume` / `max_pii_risk` 不匹配返 PRIVACY_VIOLATION 早于 schema 校验 + `ToolSpec.metadata={"affects_preview": bool}` 新增字段 + `affects_preview()` helper + `retrieve_evidence` 标 `affects_preview=True`(output 真正注入 build_sections → rewrite_highlights) + `_build_step_context(tool_name, has_jd, has_external_resume)` 派发 context + `agent_summary.tools_used` 升级为有效语义只列 `affects_preview=True` 且 `status=success` 的工具 + 错误描述只含字段名 + 类型名 + 权限名 不含 args / JD / bullet 原文(spec §6.4 隐私) + 441 老测试零回退 + 老路径 `enable_agent_workflow=False` 字节级一致 `agent_summary` 字段只在 workflow 路径加 + **不挂 pre-push hook**(spec §12 #3 默认手动脚本) + **不引入新 LLM 调用 / 不引入新依赖**(纯 stdlib + dataclass) + **不实际消费** external_resume / 不把 eval 挂 pre-push(留 Phase 3 / Phase 5) + **不实施 GUI Agent 面板**(留 Phase 4 等用户明确启动))
