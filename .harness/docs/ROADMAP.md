...
| 测试 | `backend/tests/` **863 pytest** (R6-B Phase 5 收尾时活跃基线,2026-06-30 `D:\python3.11\python.exe -m pytest tests/ -q` 实测;历史快照:R5-E 683 / R6-A Phase 2 723 / R6-A Phase 5 729 / R6-A Phase 4 739 / R6-B Phase 2 768 / R6-B Phase 3 809 / R6-B Phase 4 840 / R6-B Phase 5 863;新增覆盖含 `test_interview_agent.py` slot_meta + policy integration,`test_interview_api.py` mode switch + draft verification,`test_interview_policy.py`, `test_interview_verifier.py`, `test_interview_eval.py` extractor compare) |
| 前端入口 | `frontend/src/App.vue` |
| 设计文档 | `.harness/docs/` (含 `round5-c-agent-capability-spec.md` 5 phase 已 ✅ + `round5-d-llm-eval-spec.md` 6 phase 已 ✅ + §12 手动 live eval 安全跑法 + **`round5-e-prompt-optimization-spec.md` 4 phase 已 ✅** + §11 Phase 4 收尾记录 + **`round6-b-interview-agent-intelligence-spec.md` R6-B 7 phase 已 ✅** + §16 收尾记录) |
...
_最后更新:2026-06-30 R6-B Phase 0+1+2+3+4+5+6 + Phase 7 文档收尾全部完成(`717e47c docs(round6-b): sync active baseline 863 after phase 5`);R6-B 在 R6-A Phase 4 已上线 LLM slot extraction 的基础上补齐可信增强层:slot_meta provenance / API mode 开关 / deterministic confidence-aware policy / draft verifier / eval compare / frontend 最小呈现;后端全量 **863 passed + 0 skipped**,前端 `vue-tsc --noEmit` 0 error + `npm run build` 成功,offline compare 报告隐私自检通过;默认路径仍关闭智能抽取(`enable_interview_llm=False`)并保持 rules 行为兼容;真实 LLM eval 仍是手动流程,不进入 pre-push / 默认启动。下一轮 P1 候选:优先 R6-C live eval 收益验证(用户跑 10+ 轮真实对话 + 配置 LLM key 后执行 `scripts/evaluate_interview_agent.py --mode live --extractor compare`),再按结果决定是否机械拆分 `core/interview_llm.py` 或推进 R5-F prompt rollout。_
