...
- **背景**:**2026-06-27 user 改需求** — 把原"简历导出多格式(docx → pdf/md/html)"改成"把简历排版做到更好"。当前 5 套模板已有基础视觉差异(`classic` / `single_column` / `two_column` / `minimal` / `technical`),但模板间差异细节 / 视觉精致度 / 排版细节打磨 仍有空间
- **范围待定**:user 倾向但还没指定具体方向,以下是 5 个 brainstorm 候选:
  1. **细节打磨** — 现有 5 套模板的字体 / 字号 / 行距 / 边距 / 段距 / 标题层级 统一规范,让每套模板视觉差异更明显,中英文混排空格 / 数字单位格式 / 日期格式统一
  2. **增加新模板** — 学术 CV 风(适合读博 / 出国)/ 互联网简洁风(字节阿里 style)/ 国企端庄风 / 创意设计风 / 双语简历(中英并行)
  3. **可读性优化** — 项目描述统一 bullet 格式 / 技能标签对齐 / 时间线对齐 / 联系方式图标化
  4. **A4 排版规范** — 严格 A4 纸张大小 / 控制 1-2 页 / 自动分页 / 黑白打印友好 / 字号不小于 9pt
  5. **多列布局扩展** — 在 `two_column` 基础上扩展:左侧头像/技能,右侧项目/教育;可调列宽比例;支持跨列标题
- **触发条件**:你说"开始做 R3-M"或指定具体方向(1/2/3/4/5)
- **依赖**:`pymupdf` 已装(用于预览/导出 PDF 预览图);`python-docx` 已装(排版核心)
- **工作量**:取决于方向 — 细节打磨(中,~200 行 + 测试) / 新模板(每个 ~50 行 config + 测试) / A4 规范(小,~100 行 + 测试) / 多列扩展(中,~200 行 + 测试)
- **价值**:差异化竞争力,一份精致简历比 10 份平庸的更能拿面试
