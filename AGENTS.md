...
## Testing instructions
  ...
  - 后端 `pytest`：Round 4 收尾后有 **320 个用例**（283 R4-M baseline + 17 R5-A Phase 1 `test_agent_tools.py` + 20 R5-A Phase 1 `test_agent_workflow.py`），**320 passed + 0 skipped**（R5-A Phase 1 在 R4-M baseline 之上加：Agent 工具注册表(AGENT_TOOLS 4 个核心 + 统一 execute_agent_tool 入口 + allowlist/错误分类/隐私边界)+ 受控 Plan-and-Execute workflow(build_task_graph 确定性 + run_agent_workflow 失败时降级旧路径 + enable_agent_workflow=False 字节级一致 + AgentStep 不存原文 PII)+ generator/api 加 enable_agent_workflow kwarg(默认 False，R4-F/R4-M/R3-I/R3-M.3 老路径完全不动)）
