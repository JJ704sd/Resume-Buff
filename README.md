...
| **R5-C Phase 1 eval request_id 关联 + fallback taxonomy 5 类**(`scripts/evaluate_agent_workflow.py` 优先读 `preview["agent_summary"]["request_id"]` 替代脆弱的 JSONL 末尾反推;新增 fallback_category 5 类常量:`none` / `llm_disabled_fallback` / `tool_error_fallback` / `schema_retry_fallback` / `workflow_abort_fallback`;报告 `AI岗位JD库_agent_eval报告.md` 新增 "6.1 fallback taxonomy 摘要" 段含全局分布表;**eval 报告 request_id 短串只截前 4 字符不输出完整 uuid**(spec §2.4 隐私)) | ✅ Round 5-C Phase 1 |
| **R5-C Phase 2 外部简历进入 Agent workflow**(`PreviewRequest` / `GenerateRequest` 加 `external_resume_text: str \| None = None` 字段, 老路径完全忽略字节级一致;`core/agent_workflow.py` `run_agent_workflow` 接受 `external_resume_text` kwarg, `has_external_resume` 以 `external_resume_text` 非空为真实依据;任务图 `has_external_resume=True` 时插入 `parse_external_resume` + `compare_resume_jd` 两步(spec §3.3);`core/jd_parser.py` 新增 `parse_external_resume(text)` 输出 `{profile: {char_count, paragraph_count}, keywords: list[str]}` 不含段落原文, 与 `compare_resume_jd(text, jd_text, target_role, materials)` 输出 `{have_keywords, need_keywords, materials_can_cover, resume_only_keywords, suggestions, counts: {have, need, materials_can_cover, resume_only}}` 4 维摘要;workflow preview 返回值新增 `external_resume_perspective` 字段, 老路径完全不含此字段;**隐私边界**:`ToolResult.output` 不存原文(parse_external_resume 只返统计 + 关键词, compare_resume_jd 只返 4 维摘要 + counts + suggestions), JSONL trace 只写 `input_size/output_size`, `agent_summary` 不含简历原文;**17 个新 pytest**(`tests/test_r5c_phase2_external_resume.py`);**不挂 pre-push hook** / **不引入新 LLM 调用 / 不引入新依赖** / **不实际消费** external_resume 进改写(留 Phase 3)) | ✅ Round 5-C Phase 2 |
| **R5-C Phase 3 per-bullet 真实评估数据流**(`core/agent_workflow.py` 新增 `_evaluate_top_bullets(materials, target_role, jd_focus, jd_context, *, top_projects=3, bullets_per_project=3)` helper — top projects 来自 `ROLE_CONFIG["preferred_project_ids"]`, 有 jd_context 时用 `rank_projects` 按命中数重排, 跟 `build_sections` 同源 × top bullets 来自 `_pick_highlights(proj, target_role)` 前 3 条, 逐条跑 `evaluate_bullet_jd_match` 纯函数聚合, 输出 list[dict] 含 7 字段 `project_id` / `bullet_index` / `matched_keywords` / `missing_keywords` / `matched_count` / `missing_count` / `suggestion` **不含 bullet 原文**); 任务图 `evaluate_bullet_jd_match` step 触发条件由 `enable_function_calling and has_jd` 改为 `has_jd` (不再依赖 FC, spec §4.2); workflow 主循环执行 evaluate step 时走批量 helper 而非单 representative bullet, 单 step 1 条 trace; workflow preview 返回值新增 `bullet_evaluations` 字段(老路径不含 — 字节级一致); `evaluate_bullet_jd_match` 仍 `affects_preview=False` (诊断输出, 不注 build_sections / rewrite_highlights); 无 JD / 项目无 bullets / 批量失败 → `bullet_evaluations = []`; 单条 bullet 评估失败 → 该条用空 matched/missing schema 占位不阻断;**10 个新 pytest**(`tests/test_r5c_phase3_bullet_evaluation.py`); **baseline 3 个修复**(`test_each_tool_has_required_fields` 接受 2 个新 permission + `test_with_external_resume_adds_parse_step` / `test_true_adds_external_resume_step` 从 `tool=None` 占位改为 `tool="parse_external_resume"` 真工具断言); 全量 **530 passed + 0 skipped**; **不挂 pre-push hook**; **不引入新 LLM 调用 / 不引入新依赖**; **不破老路径字节级一致** — `enable_agent_workflow=False` 路径不含 `bullet_evaluations` 字段) | ✅ Round 5-C Phase 3 |
| **R5-C Phase 4 前端 Agent 面板契约**(`frontend/src/api/index.ts` 加 `AgentSummary` / `EvidenceSummary` / `ExternalResumePerspective` / `BulletEvaluation` TS 类型 + `preview/generate` 透传 `enable_agent_workflow` / `session_id` / `external_resume_text`;前端 App.vue 默认收起"Agent Workflow 诊断"面板, 默认 `enable_agent_workflow=False`, 用户不勾时不渲染面板不传字段, 与旧路径字节级一致;**默认 disable 所有新能力**, 等用户明确开启才显示摘要(隐私边界 spec §6.4)) | ✅ Round 5-C Phase 4 |
| **R5-C Phase 5 文档与回放闭环**(`scripts/replay_agent_trace.py` 加 `## Fallback Summary` 段(基于 trace 自身 status/error_type 推断 category, 涵盖 `none` / `tool_error_fallback` / `unknown` 三档, 仍只输出 schema 字段不泄漏原文)+ `--tools-used <csv>` 参数触发 `## Tools Cross-Validation (R5-C Phase 5)` 段(对账 caller 提供的 `agent_summary.tools_used`, 输出 status `ok` / `missing` / `unexpected` / `empty`); 旧 R5-A Phase 2 baseline 不破坏; `evaluate_agent_workflow.py` 仍可独立使用(request_id 关联走 agent_summary 主路径); ROADMAP / README / AGENTS 三方同步 R5-B baseline 497 → 544 全绿; **14 个新 pytest**(`tests/test_agent_trace_replay.py` 新增 3 TestReplayFallbackSummary + 7 TestReplayToolsCrossValidation + 2 TestReplayPiiSafetyR5C + 2 TestReplayRobustnessR5C); **不挂 pre-push hook**; **不引入新 LLM 调用 / 不引入新依赖**(纯 stdlib)) | ✅ Round 5-C Phase 5 |
...
- **Round 5-A Agent 增强(Phase 1 + Phase 2 + Phase 3 + Phase 4)**:
  ...
  - **R5-C Phase 1 eval request_id 关联 + fallback taxonomy**(`scripts/evaluate_agent_workflow.py` 优先读 `preview["agent_summary"]["request_id"]` 替代脆弱的 JSONL 末尾反推(spec §2.1)+ 新增 fallback_category 5 类常量(`none` / `llm_disabled_fallback` / `tool_error_fallback` / `schema_retry_fallback` / `workflow_abort_fallback`)+ 报告 `AI岗位JD库_agent_eval报告.md` 新增 "6.1 fallback taxonomy 摘要" 段含全局分布表;报告 request_id 短串只截前 4 字符(spec §2.4 隐私))
  - **R5-C Phase 2 外部简历进入 Agent workflow**(`PreviewRequest` / `GenerateRequest` 加 `external_resume_text: str \| None = None` 字段, 老路径完全忽略字节级一致; `core/agent_workflow.py` `run_agent_workflow` 接受 `external_resume_text` kwarg, `has_external_resume` 以 `external_resume_text` 非空为真实依据;任务图 `has_external_resume=True` 时插入 `parse_external_resume` + `compare_resume_jd` 两步(spec §3.3); `core/jd_parser.py` 新增 `parse_external_resume(text)` 输出 `{profile: {char_count, paragraph_count}, keywords: list[str]}` 不含段落原文, 与 `compare_resume_jd(text, jd_text, target_role, materials)` 输出 `{have_keywords, need_keywords, materials_can_cover, resume_only_keywords, suggestions, counts}` 4 维摘要;workflow preview 返回值新增 `external_resume_perspective` 字段, 老路径完全不含此字段;**17 个新 pytest** (`tests/test_r5c_phase2_external_resume.py`); Phase 2 commit `c6ead64`)
  - **R5-C Phase 3 per-bullet 真实评估数据流**(`core/agent_workflow.py` 新增 `_evaluate_top_bullets(materials, target_role, jd_focus, jd_context, *, top_projects=3, bullets_per_project=3)` helper — 选 top projects × top bullets, 逐条跑 `evaluate_bullet_jd_match` 纯函数聚合, 输出 list[dict] 含 7 字段 **不含 bullet 原文**;任务图 `evaluate_bullet_jd_match` step 触发条件由 `enable_function_calling and has_jd` 改为 `has_jd` (不再依赖 FC, spec §4.2);workflow preview 返回值新增 `bullet_evaluations` 字段(老路径不含 — 字节级一致);`evaluate_bullet_jd_match` 仍 `affects_preview=False` (spec §4.4);**10 个新 pytest** (`tests/test_r5c_phase3_bullet_evaluation.py`); baseline 3 个修复;全量 487 → **530 全绿**)
  - **R5-C Phase 4 前端 Agent 面板契约**(`frontend/src/api/index.ts` 加 `AgentSummary` / `EvidenceSummary` / `ExternalResumePerspective` / `BulletEvaluation` TS 类型 + `preview/generate` 透传 `enable_agent_workflow` / `session_id` / `external_resume_text`;前端 App.vue 默认收起"Agent Workflow 诊断"面板, 默认 `enable_agent_workflow=False`, 用户不勾时不渲染面板不传字段, 与旧路径字节级一致)
  - **R5-C Phase 5 文档与回放闭环**(`scripts/replay_agent_trace.py` 加 `## Fallback Summary` 段(基于 trace 自身 status/error_type 推断 category, 涵盖 `none` / `tool_error_fallback` / `unknown` 三档)+ `--tools-used <csv>` 参数触发 `## Tools Cross-Validation (R5-C Phase 5)` 段(对账 caller 提供的 `agent_summary.tools_used`, 输出 status `ok` / `missing` / `unexpected` / `empty`);ROADMAP / README / AGENTS 三方同步 R5-B baseline 497 → **544 全绿**;**14 个新 pytest** (`tests/test_agent_trace_replay.py` 新增 3 TestReplayFallbackSummary + 7 TestReplayToolsCrossValidation + 2 TestReplayPiiSafetyR5C + 2 TestReplayRobustnessR5C);不挂 pre-push hook / 不引入新 LLM 调用 / 不引入新依赖)
...
- **R5-D 真实 LLM 接入**: 当用户提供真实 LLM key 时,跑完整 12 JD × 4 开关对照得出 schema pass rate / fallback rate / latency 等真实指标(spec Phase 4 实际收益评估)
- **R5-E Vector DB RAG 升级**: 把 `core/evidence.py` 的 lexical retrieval 升级为真 embedding API 检索(需用户明确启动 + 评估成本)
