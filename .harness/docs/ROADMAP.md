...
| 核心域 | `backend/core/generator.py` + `backend/core/jd_parser.py` + `backend/core/llm_rewriter.py` + `backend/core/jd_ranker.py` + **R4 session** `backend/core/session.py` + **R5-A** `backend/core/agent_tools.py` + `backend/core/agent_workflow.py` + **`backend/core/evidence.py`** + **R5-E** `PROMPT_VERSIONS` 注册表 (4 key) + `_resolve_prompt_version` / `_select_system_prompt` 纯函数 + `_call_judge` / `_validate_judge_payload` judge helpers in `core/llm_rewriter.py` |
| API | `backend/api/resume.py` + `backend/api/jd.py` |
| 测试 | `backend/tests/` **683 pytest** (R5-E Phase 4 收尾时活跃基线,2026-06-29 `D:\python3.11\python.exe -m pytest tests/ --collect-only` 实测;R5-D 收尾时为 596;计算口径沿用 R5-E:R5-D 596 baseline + **31 R5-E Phase 1** NEW FILE `test_prompt_versioning.py` + **28 R5-E Phase 2** in NEW FILE `test_prompt_eval.py` (A/B 评测 + 隐私边界) + **28 R5-E Phase 3** in `test_prompt_eval.py` (judge schema + 网络失败 + offline 强制 disabled)) |
...
_最后更新:2026-06-29 R5-E Prompt 版本化 + A/B Eval 闭环 + 文档收尾 全部完成(4 phase + Phase 4 收尾落地,**不 rollout winner**);R5-D 6 phase + 文档收尾 `docs(round5-d): document live eval workflow` 已落;R5-A Phase 1-4 + closeout 已通过 PR #4 合并到 main(merge `12dfcf1`);R5-B Phase 2A 已通过 PR #6 合并到 main(merge `09a2704`);R5-C Phase 1-5 全部落地 baseline 530→544 pytest 全绿(R5-C 收尾时快照);R5-D Phase 0-5 全部落地 baseline 547→596 pytest 全绿(R5-D 收尾时快照);R5-E Phase 0-4 全部落地 baseline 596→683 pytest 全绿(R5-E 收尾时点,2026-06-29 `D:\python3.11\python.exe -m pytest tests/ --collect-only` 实测);R5-E 5 commit 顺序:`7e92789` phase 1 / `1c65bf9` spec / `fae76c4` phase 2+3 / `51bf33b` offline 报告 / Phase 4 closeout `docs(round5-e): close prompt ab harness round`;**默认 prompt 仍为 v2-baseline,3 个候选 prompt 仍属实验,本轮不切 winner**;由 orchestrator 维护;下一轮候选二选一:(a) R5-E closeout / R5-F prompt rollout 基于 live A/B 报告决策默认切换;(b) R5-F embedding RAG 升级 evidence retrieval(若 live 报告显示 hallucination 仍高,优先此路径不切 prompt)。R5-G 前端 dashboard 留 P2(用户偏好 GUI 实施任务默认暂停,等用户明确启动)。_
